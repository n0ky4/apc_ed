...
/**
 * @brief Calcula o Produto de Hadamard dos vetores `u` e `v` e o armazena no vetor `saida`
 * @param n Quantidade de elementos
 * @param u Vetor u
 * @param v Vetor v
 * @param saida Vetor de saída
 */
void calcular_produto_hadamard(const int n, const int *u, const int *v, int *saida) {
    for (int i = 0; i < n; i++) 
        saida[i] = u[i] * v[i];
}
...
/**
 * @brief Libera memória de 3 vetores especificados
 * @param a Vetor 1
 * @param b Vetor 2
 * @param c Vetor 3
 */
void liberar_memoria(int *a, int *b, int *c) {
    free(a);
    free(b);
    free(c);
}
...
int main() {
    ...
    // Alocar vetores u, v e hadamard
    int *u = malloc(n * sizeof(int));
    int *v = malloc(n * sizeof(int));
    int *hadamard = malloc(n * sizeof(int));

    if (u == NULL || v == NULL || hadamard == NULL) {
        liberar_memoria(u, v, hadamard);
    ...
    calcular_produto_hadamard(n, u, v, hadamard);
    // Imprimí-lo:
    imprimir_vetor("Produto de Hadamard", n, hadamard);
